...
# DASHBOARD TITLE
# ---------------------------------------------------
st.title(":vertical_traffic_light: SLA Not Met Executive Dashboard")

total = len(filtered_df)

# ---------------------------------------------------
# KPI SECTION (MoM Removed)
# ---------------------------------------------------
...
    col2.metric("Latest Month", latest_month.strftime("%b-%Y"), f"{latest_value:,}")
    col3.metric("Top RCA", top_rca.index[0] if not top_rca.empty else "N/A")
...
    y="Count",
    markers=True,
    title=":chart_with_upwards_trend: Monthly Trend"
)
...
    y="Count",
    color="RCA",
    title=":bar_chart: Month vs RCA Distribution"
)
...
    y="RCA",
    orientation="h",
    title=":pushpin: RCA Contribution"
)
